...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    UITabBarController *tb = (UITabBarController *)self.window.rootViewController; GoogleSearchViewController *toViewController0 =
    [tb.viewControllers objectAtIndex:0]; toViewController0.searchResults = self.searchResults;
    GoogleResultsViewController *toViewController1 =
    [[[tb.viewControllers objectAtIndex:1] childViewControllers] objectAtIndex:0];
...
- (void) switchWebView {
    UITabBarController *tb = (UITabBarController *)self.window.rootViewController; UIView * fromView = tb.selectedViewController.view;
    UIView * toView = [[tb.viewControllers objectAtIndex:2] view];
    // Transition using a page curl.
    [UIView transitionFromView:fromView
                        toView:toView duration:0.5 options:(2 > tb.selectedIndex ?
     ...
     -(void) switchResultView {
         ...
         [UIView transitionFromView:fromView
                            ...
                            options:(1 > tb.selectedIndex ? UIViewAnimationOptionTransitionCurlUp : UIViewAnimationOptionTransitionCurlDown)
                         completion:^(BOOL finished) { if (finished) {
             tb.selectedIndex = 1; }
                         }]; }
